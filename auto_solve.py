...
class AutoSolve():
    def __init__(self, img_filename, model_filename, model_size):
        self.img_filename = img_filename
        self.model_filename = model_filename
        self.model_size = model_size

    def auto_solve(self, track):
        raw_picture = self.load_picture(self.img_filename)
        if raw_picture is None:
            return None
        model = self.load_model(self.model_filename)
        fixed_picture = self.preprocessing_images(raw_picture, show=True)
        ...
        squares_num_list = self.get_num_list(squares_img_list, model)
        norm_board, norm_board2 = self.normalize_board(squares_num_list), self.normalize_board(squares_num_list)
        return self.solve_sudoku(sudoku_board, norm_board, norm_board2, show=True)
    ...
    def get_num_list(self, list_images, model):
...
if __name__ == '__main__':
    ...
    sudoku = AutoSolve(path, model_path, size_of_digit_img)

    s = sudoku.auto_solve(tracking)
    if not s:
